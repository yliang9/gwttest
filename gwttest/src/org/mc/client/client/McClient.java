...
public class McClient {
	...
	private MenuBar createMenu() {
		...
		MenuItem homeItem = new MenuItem("Home", command2);

		menutop.addItem("File", fileMenu); // Creates item and adds menutwo
	//	menutop.addItem(homeItem);
	...
	private Widget getConfigPanel(Mart mart, McConfig config) {
		...
		final CellTable<List<String>> stable = this.getPropertyTable();
		...
		AsyncCallback<McConfig> callback = new AsyncCallback<McConfig>() {
		      ...
		      public void onSuccess(McConfig result) {
		    	  //get rootcontainer;
		    	  getRootContainer(spsource,result, stable);
		      }
		};		
		service.getMasterConfig(mart, callback);
		
		TreeViewModel model = new ConfigTreeModel(null,null,stable);
	    ...
	    //((Object) treeSource).setEmptyTreeItemMessage(SafeHtmlUtils.fromString("data loading ..."));
		//create source 		
		...
		ScrollPanel sproPanel = new ScrollPanel();
		sproPanel.add(stable);
		sourcePanel.add(sproPanel);
		return configPanel;
	}
	
	private void getRootContainer(final ScrollPanel sp, final McConfig config, final CellTable<List<String>> table) {
		ConfiguratorServiceAsync service = GWT.create(ConfiguratorService.class);
		AsyncCallback<Container> callback = new AsyncCallback<Container>() {
		      ...
		      public void onSuccess(Container result) {
		    	  resetTree(sp,config,result, table);
		      }
		};		
		service.getRootContainerRecursively(config, callback); 
	}
	
	private void resetTree(ScrollPanel sp, McConfig config, Container rootContainer, CellTable<List<String>> table) {		
		sp.clear();
		TreeViewModel model = new ConfigTreeModel(config,rootContainer,table);
		CellTree tree = new CellTree(model, "root");
		sp.add(tree);
//		TreeNode root = tree.getRootTreeNode();
		
//		model.getNodeInfo(config).getSelectionModel().setSelected((?)root, true);
	}
	
	private CellTable<List<String>> getPropertyTable() {
		CellTable<List<String>> table = new CellTable<List<String>>();
		table.setWidth("100%");
		TextColumn<List<String>> apcolumn = new TextColumn<List<String>>() {
			@Override
		...
		TextColumn<List<String>> sourcecolumn = new TextColumn<List<String>>() {
			@Override
			public String getValue(List<String> object) {
				return object.get(1);
			}			
		};
		table.addColumn(apcolumn,"Property");
		table.addColumn(sourcecolumn,"Value");
		return table;
	}
	
	public Widget createMcFrame() {
